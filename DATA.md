...
### Этап 1: Приоритезация (Заполнение таблиц `product`, `queue`)

Данные, собранные на этапе Приоритезации, отражающие состояние товаров в очереди (бэклоге). Эти данные используются для создания или обновления записей в таблицах `product` и `queue`.

#### Таблица: `product` (создание/обновление)

| id (placeholder) | name                                       | description | price | created_at | updated_at |
|------------------|--------------------------------------------|-------------|-------|------------|------------|
| prod_1 | кабель rj9 / rj10 / rj11; male plug connector; 4P4 plug RJ9/RJ10 |  |  |  |  |
| prod_2 | Аудио кабель 4-полюсный разъем стерео       |  |  |  |  |
| prod_3 | Клей Момент                                |  |  |  |  |
| prod_4 | Пневматическая быстроразъёмная муфта PCF 6-04 |  |  |  |  |
| prod_5 | RGB светодиодная лента HugSunLight         |  |  |  |  |
| prod_6 | Ручной пресс для чеснока                   |  |  |  |  |
| prod_7 | Медный провод 0,1, ~ длина 650 м -400 ПРОМОКОД COMBO |  |  |  |  |
| prod_8 | Трусы                                      |  |  |  |  |

#### Таблица: `queue` (создание/обновление)

| id (placeholder) | product_id (FK) | status_id (FK) | priority_id (FK) | created_at | assigned_to_user_account_id (FK) | note |
|------------------|-----------------|----------------|------------------|------------|----------------------------------|------|
| queue_item_1 | prod_1 | 42 | 2 | [Timestamp] | [User ID] | 22 мая открыть спор для возврата средств |
| queue_item_2 | prod_2 | 42 | 2 | [Timestamp] | [User ID] | 22 мая открыть спор для возврата средств |
| queue_item_3 | prod_3 | 39 | 2 | [Timestamp] | [User ID] |  |
| queue_item_4 | prod_4 | 41 | 3 | [Timestamp] | [User ID] | Продавец потребовал отмены. Получено 100 баллов |
| queue_item_5 | prod_5 | 39 | 3 | [Timestamp] | [User ID] |  |
| queue_item_6 | prod_6 | 39 | 2 | [Timestamp] | [User ID] |  |
| queue_item_7 | prod_7 | 39 | 3 | [Timestamp] | [User ID] | -400 ПРОМОКОД COMBO |
| queue_item_8 | prod_8 | 40 | 1 | [Timestamp] | [User ID] |  |

### Этап 2: Поиск/Эффективность (Заполнение таблиц `offer`, `competitor_price`)

Данные, собранные в ходе анализа экономической выгоды для выбранных товаров. Эти данные используются для создания или обновления записей в таблицах `offer` и `competitor_price`.

#### Таблица: `offer` (создание/обновление)

| id (placeholder) | product_id (FK) | channel | price | status (Enum/FK) | available | acquisition_cost |
|------------------|-----------------|---------|-------|------------------|-----------|------------------|
| offer_1 | prod_5 | AliExpress | 500.00 | [Status ID for available offer] | TRUE |  |

#### Таблица: `competitor_price` (создание/обновление)

| id (placeholder) | product_id (FK) | competitor_name | price | recorded_at |
|------------------|-----------------|-----------------|-------|-------------|
| comp_price_1 | prod_5 | Ozon | 600.00 | [Timestamp] |
...
| id (placeholder) | user_account_id (FK) | created_at | updated_at | total_amount | strategy |
|------------------|----------------------|------------|------------|--------------|----------|
...
| id (placeholder) | basket_id (FK) | product_id (FK) | qty | added_at | price_at_add | subtotal | queue_id (FK) |
|------------------|----------------|-----------------|-----|----------|--------------|----------|---------------|
| basket_item_1 | basket_1 | prod_5 | 1 | [Timestamp] | 500.00 | 500.00 | queue_item_5 |
| basket_item_2 | basket_1 | prod_7 | 1 | [Timestamp] | [Price of Медный провод] | [Subtotal] | queue_item_7 |
...
| id (placeholder) | order_id (FK) | pay_method_id (FK) | paid_at | amount | cashback_received |
|------------------|---------------|--------------------|---------|--------|-------------------|
|  |  |  |  |  |  |

#### Таблица: `pay_method` (создание/обновление, если добавляется новый метод)

| id (placeholder) | name | wallet_id (FK) | currency_id (FK) | details |
|------------------|------|----------------|------------------|---------|
|  |  |  |  |  |

#### Таблица: `wallet` (создание/обновление, если добавляется новый кошелек)

| id (placeholder) | name | balance | currency_id (FK) |
|------------------|------|---------|------------------|
|  |  |  |  |

### Этап 5: Регистрация/Заказ (Заполнение таблиц `order`, `shipment`, `user_account`, `address`)

Финальные данные заказа, включая информацию о пользователе, адресах и отгрузке. Эти данные используются для создания или обновления записей в таблицах `order`, `shipment`, `user_account`, и `address`, а также финального связывания данных из предыдущих этапов.

#### Таблица: `user_account` (создание/обновление)

| id (placeholder) | username | password | email | created_at | default_shipping_address_id (FK) |
|------------------|----------|----------|-------|------------|----------------------------------|
|  |  |  |  |  |  |

#### Таблица: `address` (создание/обновление)

| id (placeholder) | street | city | state_province | postal_code | country | building_apt | details |
|------------------|--------|------|----------------|-------------|---------|--------------|---------|
|  |  |  |  |  |  |  |  |

#### Таблица: `order` (создание/обновление)

| id (placeholder) | user_account_id (FK) | order_date | status_id (FK) | total_amount | shipping_address_id (FK) | billing_address_id (FK) |
|------------------|----------------------|------------|----------------|--------------|--------------------------|-------------------------|
|  |  |  |  |  |  |  |

#### Таблица: `order_item` (создание/обновление - финальные данные элементов корзины)

| id (placeholder) | order_id (FK) | product_id (FK) | qty | price_per_item | subtotal | cost_of_goods_sold | sale_price | discount_amount | discount_description |
|------------------|---------------|-----------------|-----|----------------|----------|--------------------|------------|-----------------|----------------------|
|  |  |  |  |  |  |  |  |  |  |

#### Таблица: `shipment` (создание/обновление)

| id (placeholder) | order_id (FK) | shipped_at | delivered_at | tracking_number | status_id (FK) | shipping_cost | shipping_address_id (FK) |
|------------------|---------------|------------|--------------|-----------------|----------------|---------------|--------------------------|
|  |  |  |  |  |  |  |  |
